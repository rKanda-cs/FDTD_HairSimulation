...
//					if (i < Npml || i > mField->getNpx() - Npml - 1 || j < Npml || j > mField->getNpy() - Npml - 1)
						HX(i, j, +1) = BHYP(i, j) * BHYM(i, j) * HX(i, j, 0) 
							- BHYP(i, j)*CHXLY(i, j)*(R_P*(EZ(i, j+1, +1) - EZ(i, j, +1)) + R_M*Dy2_n(Ez, i, j+1));	//Hxの計算 Hx(i, j+1/2) -> Hx[i,j]
...
//					if (i < Npml || i > mField->getNpx() - Npml - 1 || j < Npml || j > mField->getNpy() - Npml - 1)
						HY(i, j, +1) = BHXP(i, j) * BHXM(i, j) * HY(i, j, 0) 
							+ BHXP(i, j)*CHYLX(i, j)*(R_P*(EZ(i+1, j, +1) - EZ(i, j, +1)) + R_M*Dx2_n(Ez, i+1, j));	//Hyの計算 Hy(i+1/2, j) -> Hy[i,j]
